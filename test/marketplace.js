...
describe("Marketplace contract", function () {
  ...
  beforeEach(async function () {
    ...
    await connectedWeth.approve(marketplace.address, ethers.constants.MaxUint256);
  ...
  it("should allow you to offer an NFT", async function () {
    this.marketplace = this.marketplace.connect(this.account1);
    await expect(this.marketplace.makeOffer(this.dummyNFT.address, 1, ethers.utils.parseEther("1")))
      .to.emit(this.marketplace, 'NewOffer')
      .withArgs(this.account1.address, ethers.utils.parseEther("1"), this.dummyNFT.address, 1);
  ...
  it("should takeOffer", async function () {
    ...
    this.marketplace = this.marketplace.connect(this.account3);
    await this.marketplace.makeBid(this.dummyNFT.address, 1, ethers.utils.parseEther("0.5"));
    ...
    await expect(this.marketplace.takeOffer(this.dummyNFT.address, 1, {value: ethers.utils.parseEther("1")}))
      .to.emit(this.marketplace, 'Sale')
      .withArgs(this.account2.address, this.account1.address,
        ethers.utils.parseEther("1"), this.dummyNFT.address, 1);
  ...
  it("should makeBid", async function () {
    this.marketplace = this.marketplace.connect(this.account1);
    await expect(this.marketplace.makeBid(this.dummyNFT.address, 2, ethers.utils.parseEther("1")))
      .to.emit(this.marketplace, 'NewBid')
      .withArgs(this.account1.address, ethers.utils.parseEther("1"), this.dummyNFT.address, 2);
  ...
  it("should revert takeBid if you don't own NFT", async function () {
    this.marketplace = this.marketplace.connect(this.account1);
    await this.marketplace.makeBid(this.dummyNFT.address, 2, ethers.utils.parseEther("1"));
    this.marketplace = this.marketplace.connect(this.account3);
    await expect(this.marketplace.takeBid(this.dummyNFT.address, 2)).to.be.revertedWith(
  ...
  it("should takeBid", async function () {
    ...
    this.marketplace = this.marketplace.connect(this.account2);
    await this.marketplace.makeOffer(this.dummyNFT.address, 2, ethers.utils.parseEther("2"));
    ...
    await expect(this.marketplace.takeBid(this.dummyNFT.address, 2))
      .to.emit(this.marketplace, 'Sale')
      .withArgs(this.account1.address, this.account2.address,
        ethers.utils.parseEther("1"), this.dummyNFT.address, 2);
    ...
    expect(await weth.balanceOf(this.account2.address)).to.be.equal(ethers.utils.parseEther("1"));
    // Check balance of account 1 is 1 WETH less.
    expect(await weth.balanceOf(this.account1.address)).to.be.equal(ethers.utils.parseEther("999"));
    // There should be no more offer for that NFT
  ...
  it("should revert removeOffer if you don't own the NFT", async function () {
    this.marketplace = this.marketplace.connect(this.account1);
    await expect(this.marketplace.removeOffer(this.dummyNFT.address, 2)).to.be.revertedWith(
      "You do not own this NFT.");
  });
  ...
  it("should revert removeBid if you aren't the bidder", async function () {
    ...
    await this.marketplace.makeBid(this.dummyNFT.address, 2, ethers.utils.parseEther("1"));
    this.marketplace = this.marketplace.connect(this.account3);
    await expect(this.marketplace.removeBid(this.dummyNFT.address, 2)).to.be.revertedWith(
