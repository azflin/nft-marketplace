...
describe('Marketplace contract', function () {
  ...
  it('should only allow owner to call withdraw', async function () {
    const nonOwner = this.account2.address;
    this.marketplace = this.marketplace.connect(nonOwner);
    expect(this.marketplace.withdraw()).to.be.reverted;
  });

  // it('should increase owner balance after calling withdraw function', async function () {
  //   const owner = this.account1.address;
  //   await this.weth.approve(this.marketplace.address, 0);
  //   this.marketplace = this.marketplace.connect(owner);
  //   expect(await this.marketplace.withdraw());
  // });
});
